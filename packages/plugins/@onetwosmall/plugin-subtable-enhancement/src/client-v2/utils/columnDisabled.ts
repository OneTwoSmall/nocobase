...
export function syncColumnDisabled(
  ...
  ownDisabledMap: WeakMap<object, boolean>,
): void {
  for (const column of columns) {
    if (!column) continue;
    if (!blockDisabled) {
      if (ownDisabledMap.has(column)) {
        const own = ownDisabledMap.get(column);
        if (!!column.props?.disabled !== own) {
          column.setProps?.({ disabled: own });
        }
        ownDisabledMap.delete(column);
      }
      continue;
    }
    if (!ownDisabledMap.has(column)) {
      ownDisabledMap.set(column, !!column.props?.disabled);
    }
